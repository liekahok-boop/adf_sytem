...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['invoice_logo'])) {
    ...
    if ($file['error'] === UPLOAD_ERR_OK) {
        ...
        if (!in_array($file['type'], $allowedTypes)) {
            ...
            $filename = 'invoice_logo_' . time() . '.' . $extension;
            $uploadPath = $uploadDir . $filename;
            
            // Delete old logo if exists
            $currentLogo = $db->fetchOne("SELECT setting_value FROM settings WHERE setting_key = 'invoice_logo'");
            ...
            if (move_uploaded_file($file['tmp_name'], $uploadPath)) {
                // Update database
                $exists = $db->fetchOne("SELECT COUNT(*) as count FROM settings WHERE setting_key = 'invoice_logo'");
...
if (isset($_GET['delete_invoice_logo']) && $_GET['delete_invoice_logo'] === '1') {
    ...
    if ($currentLogo) {
        ...
        $db->query("DELETE FROM settings WHERE setting_key = 'invoice_logo'");
            ...
            <div style="display: grid; grid-template-columns: auto 1fr; gap: 1.5rem; align-items: start;">
                ...
                <div style="text-align: center;">
                    <div style="width: 120px; height: 120px; border: 2px dashed var(--bg-tertiary); border-radius: 0.5rem; display: flex; align-items: center; justify-content: center; background: var(--bg-primary); overflow: hidden;">
                        <?php 
                        $invoiceLogo = $db->fetchOne("SELECT setting_value FROM settings WHERE setting_key = 'invoice_logo'");
            ...
            <div style="padding: 1rem;">
                ...
                <div style="background: white; padding: 1.5rem; border-radius: 0.5rem; border: 1px solid #e2e8f0;">
                    <div style="display: flex; align-items: start; gap: 1rem; padding-bottom: 1rem; border-bottom: 2px solid #333;">
                        ...
                        } elseif (!empty($settings['hotel_logo'])) {
                            $displayLogo = BASE_URL . '/uploads/logos/' . $settings['hotel_logo'];
                        } elseif (!empty($settings['company_logo'])) {
                            $displayLogo = BASE_URL . '/assets/images/' . $settings['company_logo'];
                        }
                        ?>
                            ...
                            <div style="flex-shrink: 0;">
                                ...
                                <?php else: ?>
                                    <div style="width: 60px; height: 60px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); border-radius: 0.375rem; display: flex; align-items: center; justify-content: center; color: white; font-weight: 700; font-size: 1.5rem;">
                                        <?php echo substr($settings['company_name'] ?? 'N', 0, 1); ?>
                                    </div>
                                <?php endif; ?>
                            </div>
                        <?php endif; ?>
                        ...
                        <div style="flex: 1;">
                            <h2 style="font-size: 1.25rem; font-weight: 700; color: #1e293b; margin: 0 0 0.25rem 0;">
                                <?php echo $settings['company_name'] ?? 'Narayana Hotel'; ?>
                            </h2>
                            <?php if (!empty($settings['company_tagline'])): ?>
                                <p style="font-size: 0.813rem; color: #64748b; margin: 0 0 0.5rem 0;">
                                    <?php echo $settings['company_tagline']; ?>
                                </p>
                            <?php endif; ?>
                            
                            <?php if (($settings['report_show_address'] ?? '1') == '1' && !empty($settings['company_address'])): ?>
                                <p style="font-size: 0.813rem; color: #475569; margin: 0;">
                                    <strong>Alamat:</strong> <?php echo $settings['company_address']; ?>
                                </p>
                            <?php endif; ?>
                            
                            <?php if (($settings['report_show_phone'] ?? '1') == '1' && !empty($settings['company_phone'])): ?>
                                <p style="font-size: 0.813rem; color: #475569; margin: 0.125rem 0 0 0;">
                                    <strong>Telepon:</strong> <?php echo $settings['company_phone']; ?>
                                    <?php if (!empty($settings['company_email'])): ?>
                                        | <strong>Email:</strong> <?php echo $settings['company_email']; ?>
                                    <?php endif; ?>
                                </p>
                            <?php endif; ?>
                        </div>
