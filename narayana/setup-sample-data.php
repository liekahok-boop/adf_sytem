...
try {
    ...
    try {
        ...
        if ($count == 0) {
            ...
            $stmt = $conn->prepare("INSERT INTO categories (category_name, transaction_type, description, is_active) VALUES (?, ?, ?, 1)");
        ...
        $catList = $db->fetchAll("SELECT * FROM categories WHERE is_active = 1 ORDER BY transaction_type, category_name");
        echo "<table><tr><th>ID</th><th>Type</th><th>Category Name</th><th>Description</th></tr>";
        foreach ($catList as $c) {
            $typeColor = $c['transaction_type'] == 'income' ? '#10b981' : '#ef4444';
            echo "<tr><td>{$c['id']}</td><td style='color:$typeColor;font-weight:bold;'>{$c['transaction_type']}</td><td>{$c['category_name']}</td><td>{$c['description']}</td></tr>";
        }
        echo "</table>";
    ...
    try {
        ...
        if ($count < 20) {
            ...
            $incomeCategories = $db->fetchAll("SELECT id FROM categories WHERE transaction_type = 'income' AND is_active = 1");
            $expenseCategories = $db->fetchAll("SELECT id FROM categories WHERE transaction_type = 'expense' AND is_active = 1");
