...
// Get room type data
$type = $db->fetchOne("SELECT * FROM room_types WHERE id = ?", [$typeId]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Check if type name already exists (except current)
    $existing = $db->fetchOne("SELECT id FROM room_types WHERE type_name = ? AND id != ?", [$typeName, $typeId]);
    
    if ($existing) {
        setFlash('error', 'Room type dengan nama ini sudah ada!');
    } else {
        try {
            $db->update('room_types', [
                'type_name' => $typeName,
            ...
            setFlash('success', 'Room type berhasil diupdate!');
            header('Location: manage-room-types.php');
            exit;
        } catch (Exception $e) {
...
<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.5rem;">
    <div>
        ...
        <p style="color: var(--text-muted); font-size: 0.813rem;">
            <?php echo $type['type_name']; ?> - Digunakan oleh <?php echo $roomCount; ?> room(s)
        </p>
    </div>
        ...
        <form method="POST" style="padding: 1.5rem;">
            ...
            <div class="form-group">
                ...
                <input type="number" name="base_price" class="form-control" 
                       ...
                       value="<?php echo $type['base_price']; ?>"
                       placeholder="500000">
            ...
            <div class="form-group">
                ...
                <div class="color-picker-grid">
                    ...
                    ?>
                        <div class="color-option <?php echo $type['color_code'] == $color ? 'selected' : ''; ?>" 
                ...
                <input type="hidden" name="color_code" id="color_code" value="<?php echo $type['color_code']; ?>">
            </div>
            
            <div class="form-group">
                <label class="form-label">Description</label>
                <textarea name="description" class="form-control" rows="4" id="description"
                          placeholder="Deskripsi tipe kamar, fasilitas yang tersedia, dll..."><?php echo htmlspecialchars($type['description']); ?></textarea>
            </div>
    ...
    <div>
        <div class="card" style="margin-bottom: 1.5rem;">
            ...
            <div class="preview-card">
                <div class="preview-type-icon" id="previewIcon" style="background: <?php echo $type['color_code']; ?>20;">
                    <i data-feather="layers" style="width: 40px; height: 40px; color: <?php echo $type['color_code']; ?>;" id="previewIconFeather"></i>
                </div>
                ...
                <div style="background: var(--bg-tertiary); border-radius: 8px; padding: 1rem;">
                    ...
                    <div style="font-size: 0.813rem; color: var(--text-secondary); line-height: 1.5;" id="previewDesc">
                        <?php echo $type['description'] ? nl2br(htmlspecialchars($type['description'])) : 'Belum ada deskripsi'; ?>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div style="padding: 1rem;">
                ...
                <div style="background: var(--bg-secondary); border-radius: 8px; padding: 1rem;">
                    <div style="display: flex; justify-content: space-between; align-items: center;">
                        <span style="color: var(--text-muted); font-size: 0.813rem;">Rooms Using This Type</span>
                        <span style="font-size: 1.5rem; font-weight: 800; color: #6366f1;"><?php echo $roomCount; ?></span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="card">
            <div style="padding: 1rem;">
                ...
                <ul style="font-size: 0.813rem; color: var(--text-secondary); line-height: 1.6; margin: 0; padding-left: 1.25rem;">
                    <li>Type name harus unik</li>
                    <li>Perubahan harga akan apply ke semua room</li>
                    <li>Color code untuk visual di denah & report</li>
                    <li>Type yang digunakan tidak bisa dihapus</li>
                </ul>
            </div>
        </div>
    </div>
...
let currentColor = '<?php echo $type['color_code']; ?>';
...
document.getElementById('description').addEventListener('input', function() {
    document.getElementById('previewDesc').innerHTML = this.value.replace(/\n/g, '<br>') || 'Belum ada deskripsi';
});
</script>
